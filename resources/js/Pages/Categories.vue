...
export default {
    ...
    methods: {
        saveData() {
            //validate
            var data = this.$inertia.post(this.route('create.category'), {
                name: this.name,
            });
        ...
        updateData() {
            // alert(this.id)
            this.$inertia.post(this.route('update.category', {id: this.id}), {
                name: this.name,
            });
            Swal.fire({
                title: 'Success',
 ...
 <div class="grid grid-cols-1 3xl:grid-cols-2 xl:gap-4 my-4">
     <div class="bg-white shadow rounded-lg mb-4 p-4 sm:p-6 h-full">
                     <div class="flex items-center justify-between mb-4">
                        <h3 class="text-xl font-bold leading-none text-gray-900">All Categories</h3>

                        <div class="flex items-center justify-between mb-4">
                          <button @click="isOpen = true" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Add Category</button>
                        </div>
                                ...
                                <td class="border-t-0 px-4 align-middle text-sm font-normal whitespace-nowrap p-4 text-left">{{index+1}}</td>
                                 <td class="border-t-0 px-4 align-middle text-xs font-medium text-gray-900 whitespace-nowrap p-4">{{ data.title }}</td>
                                    ...
                                    <td class="border-t-0 px-4 align-middle text-xs font-medium text-gray-900 whitespace-nowrap p-4">
                                            ...
                                            <button @click="editData(data.id, data.title, data.slug)" class="px-4 py-2 text-blue-100 bg-blue-600 rounded">
                                                Edit
                                            </button> 	&nbsp;

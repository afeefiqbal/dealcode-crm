    ...
    <div class="max-w-2xl p-6 bg-white rounded-md shadow-xl">
      ...
      <div  class="grid gap-6 mb-6 md:grid-cols-2">
        ...
        <div class="justify-between">
          ...
          <input type="text" name="link" id="link"  v-model="formData.editfb_user_profule"  class="w-full max-w-lg rounded-lg border border-slate-200 px-2 py-1 hover:border-blue-500 focus:outline-none focus:ring focus:ring-blue-500/40 active:ring active:ring-blue-500/40"  />
        </div>
        <div class="justify-between">
          <label class="text-sm font-semibold">FB Group (if any)</label>
  
          <input type="text" name="link" id="link"  v-model="formData.editfb_group"  class="w-full max-w-lg rounded-lg border border-slate-200 px-2 py-1 hover:border-blue-500 focus:outline-none focus:ring focus:ring-blue-500/40 active:ring active:ring-blue-500/40"  />
        </div>
        <br />
        <div class="flex items-center justify-between mt-4">
  ...
  <div class="px-4 sm:px-6 lg:px-8">
    <div class="sm:flex sm:items-center">
      <div class="sm:flex-auto">
        <h1 class="text-xl font-semibold text-gray-900">Reseller Lists</h1>
            ...
            <table class="min-w-full divide-y divide-gray-300">
              <thead class="bg-gray-50">
                <tr class="divide-x divide-gray-200">
                  ...
                  <th scope="col" class="px-4 py-3.5 text-left text-sm font-semibold text-gray-900">FB User Profile</th>
                  <th scope="col" class="px-4 py-3.5 text-left text-sm font-semibold text-gray-900">FB Group (If any)</th>
                  <th scope="col" class="py-3.5 pl-4 pr-4 text-left text-sm font-semibold text-gray-900 sm:pr-6">Action</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-200 bg-white">
                <tr v-for="(list, key) in resellerLists" :key="list.id" class="divide-x divide-gray-200">
                  <td class="whitespace-nowrap py-4 pl-4 pr-4 text-sm font-medium text-gray-900 sm:pl-6">{{ key+1}}</td>
                  <td class="whitespace-nowrap p-4 text-sm text-gray-500">{{ list.name }}</td>
                  <td class="whitespace-nowrap p-4 text-sm text-gray-500">{{ list.link }}</td>
                  <td class="whitespace-nowrap p-4 text-sm text-gray-500">{{ list.fb_user_profule }}</td>
                  <td class="whitespace-nowrap p-4 text-sm text-gray-500">{{ list.fb_group }}</td>
                  <td class="whitespace-nowrap py-4 pl-4 pr-4 text-sm text-gray-500 sm:pr-6">
                    <a @click="editData(list.id,list.name, list.link, list.fb_user_profule, list.fb_group)" class="text-indigo-600 hover:text-indigo-900" >
                      Edit
                    </a> &nbsp;
    ...
    const formData = useForm({
     ...
     function saveData(){
      if(formData.name == '' ){
              ...
              formData.post(route('reseller_lists.create'), {
              preserveScroll: true,
              onSuccess: () => {
                    Swal.fire({
                              title: 'Success',
                        ...
                        setTimeout(() => {
                                  Inertia.visit('reseller_lists', { preserveScroll: true });
                              }, 1000);
     ...
     function updateData(){
      if(formData.editname == '' ){
              ...
              formData.post(route('reseller_lists.update'), {
              preserveScroll: true,
              onSuccess: () => {
                    Swal.fire({
                              title: 'Success',
     ...
     function editData(id,name,link,fb_user_profule,fb_group){
       isOpens.value = true;
       formData.edit_id = id;
       formData.editname = name;
       formData.editlink = link;
       formData.editfb_user_profule = fb_user_profule;
       formData.editfb_group = fb_group;
        }
        function deleteData(ids) {
            // alert(ids)
            Inertia.post(route('reseller_lists.delete', {id: ids}));
        }
</script>

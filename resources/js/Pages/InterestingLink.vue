    ...
    <div class="max-w-2xl p-6 bg-white rounded-md shadow-xl">
      ...
      <div class="grid gap-6 mb-6 md:grid-cols-2">
        <div class="justify-between">
          ...
          <input type="text" v-model="formData.link_name" class="w-full max-w-lg rounded-lg border border-slate-200 px-2 py-1 hover:border-blue-500 focus:outline-none focus:ring focus:ring-blue-500/40 active:ring active:ring-blue-500/40" />
        </div>
        <div class="justify-between">
          <label class="text-sm font-semibold">Url</label>
  
          <input type="text" name="link" id="link"  v-model="formData.link_url"  class="w-full max-w-lg rounded-lg border border-slate-200 px-2 py-1 hover:border-blue-500 focus:outline-none focus:ring focus:ring-blue-500/40 active:ring active:ring-blue-500/40"  />
        </div>
  ...
  <div v-show="isOpens"  class="  absolute inset-0 flex items-center justify-center bg-gray-700 bg-opacity-50">
    <div class="max-w-2xl p-6 bg-white rounded-md shadow-xl">
      ...
      <div  class="grid gap-6 mb-6 md:grid-cols-2">
        <div class="justify-between">
          <label class="text-sm font-semibold">Name</label>
          <input type="text" v-model="formData.editlink_name" class="w-full max-w-lg rounded-lg border border-slate-200 px-2 py-1 hover:border-blue-500 focus:outline-none focus:ring focus:ring-blue-500/40 active:ring active:ring-blue-500/40" />
        </div>
        <div class="justify-between">
          <label class="text-sm font-semibold">Url</label>
  
          <input type="text" name="link" id="link"  v-model="formData.editlink_url"  class="w-full max-w-lg rounded-lg border border-slate-200 px-2 py-1 hover:border-blue-500 focus:outline-none focus:ring focus:ring-blue-500/40 active:ring active:ring-blue-500/40"  />
        </div>
  ...
  <div class="px-4 sm:px-6 lg:px-8">
    <div class="sm:flex sm:items-center">
      <div class="sm:flex-auto">
        <h1 class="text-xl font-semibold text-gray-900">Interesting Links</h1>
            ...
            <table class="min-w-full divide-y divide-gray-300">
              <thead class="bg-gray-50">
                <tr class="divide-x divide-gray-200">
                  ...
                  <th scope="col" class="px-4 py-3.5 text-left text-sm font-semibold text-gray-900">Link Url</th>
                  <th scope="col" class="py-3.5 pl-4 pr-4 text-left text-sm font-semibold text-gray-900 sm:pr-6">Action</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-200 bg-white">
                <tr v-for="(list, key) in interestingLinks" :key="list.id" class="divide-x divide-gray-200">
                  <td class="whitespace-nowrap py-4 pl-4 pr-4 text-sm font-medium text-gray-900 sm:pl-6">{{ key+1}}</td>
                  <td class="whitespace-nowrap p-4 text-sm text-gray-500">   {{ list.link_name }}</td>
                  <td class="whitespace-nowrap p-4 text-sm text-blue-500"> <a :href="list.link_url" target="_blank" rel="noopener noreferrer">{{list.link_url}}</a></td>
                  <td class="whitespace-nowrap py-4 pl-4 pr-4 text-sm text-gray-500 sm:pr-6">
                    <a @click="editData(list.id,list.link_name, list.link_url)" class="text-indigo-600 hover:text-indigo-900" >
                      Edit
                    </a> &nbsp;
                    <a  @click="deleteData(list.id)" href="#" class="text-red-600 hover:text-red-900" >
                      Delete
                    </a>
                  </td>
                </tr>
    ...
    const formData = useForm({
     ...
     function saveData(){
      if(formData.name == '' ){
              ...
              formData.post(route('interesting_link.store'), {
              preserveScroll: true,
              onSuccess: () => {
                    Swal.fire({
                              title: 'Success',
                        ...
                        setTimeout(() => {
                                  Inertia.visit('interesting_links', { preserveScroll: true });
                              }, 1000);
     ...
     function updateData(){
      if(formData.editlink_name == '' ){
              ...
              formData.post(route('interesting_link_update.update'), {
              preserveScroll: true,
              onSuccess: () => {
                    Swal.fire({
                              title: 'Success',
     ...
     function editData(id,name,link,p){
       isOpens.value = true;
       formData.edit_id = id;
       formData.editlink_name = name;
       formData.editlink_url = link;
        }
        function deleteData(ids) {
            // alert(ids)
            Inertia.post(route('interesting_link.delete', {id: ids}));
        }
</script>

...
export default {
    ...
    saveData(){
       ...
       if(this.name == '' || this.title == '' || this.description == '' || this.keywords == ''){
            Swal.fire({
                title: 'Error!',
       ...
       if(this.is_edit){
        //valdate form
           this.$inertia.post(this.route('meta-tags.update', {id: this.edit_id}), {
            title: this.title,
        ...
        this.$inertia.post(this.route('meta-tags.create'), {
            title: this.title,
            description: this.description,
            keywords: this.keywords,
            name: this.name,
    ...
    deleteData(ids) {
        ...
        Swal.fire({
            ...
            if (result.value) {
               this.$inertia.post(this.route('meta-tags.delete', {id: ids}));
               Swal.fire({
                title: 'Success',
 ...
 <div class="grid grid-cols-1 3xl:grid-cols-2 xl:gap-4 my-4"  v-if="datas">
     <div class="bg-white shadow rounded-lg mb-4 p-4 sm:p-6 h-full">
                     <div class="flex items-center justify-between mb-4">
                        <h3 class="text-xl font-bold leading-none text-gray-900">All Metags</h3>
                       ...
                       <table class="items-center w-full bg-transparent border-collapse">
                           ...
                           <tbody class="divide-y divide-gray-100">
                            <!-- <div v-for="user in latest_five_users" :key="user.id"> -->
                              <tr class="text-gray-500" v-for="metaTag in metaTags" :key="metaTag.id">
                                 <td class="border-t-0 px-4 align-middle text-sm font-normal  p-4 text-left">{{ metaTag.page }}</td>
                                 <td class="border-t-0 px-4 align-middle text-xs font-medium text-gray-900  p-4">{{ metaTag.title }}</td>
                                 <td class="border-t-0 px-4 align-middle text-xs font-medium text-gray-900  p-4">{{ metaTag.description }}</td>
                                 <td class="border-t-0 px-4 align-middle text-xs font-medium text-gray-900  p-4">{{ metaTag.keywords }}</td>
                                    <td class="border-t-0 px-4 align-middle text-xs font-medium text-gray-900 whitespace-nowrap p-4">
                                        <button @click="forms(metaTag, metaTag.id)" class="block pl-3 pr-4 py-2 border-l-4 border-transparent text-base font-medium text-gray-600 hover:text-gray-800 hover:bg-gray-50 hover:border-gray-300 focus:outline-none focus:text-gray-800 focus:bg-gray-50 focus:border-gray-300 transition duration-150 ease-in-out hidden sm:inline-flex ml-5 text-white bg-cyan-600 hover:bg-cyan-700 focus:ring-4 focus:ring-cyan-200 font-medium rounded-lg text-sm px-5 py-2.5 text-center items-center mr-3"><i class="fa fa-pencil-square-o">  </i>   Edit
                                        </button>
                                        <button @click="deleteData(`${metaTag.id}`)"  class="block pl-3 pr-4 py-2 border-l-4 border-transparent text-base font-medium text-gray-600 hover:text-gray-800 hover:bg-gray-50 hover:border-gray-300 focus:outline-none focus:text-gray-800 focus:bg-gray-50 focus:border-gray-300 transition duration-150 ease-in-out hidden sm:inline-flex ml-5 text-white bg-red-600 hover:bg-red-700 focus:ring-4 focus:ring-red-200 font-medium rounded-lg text-sm px-5 py-2.5 text-center items-center mr-3"><i class="fa fa-trash">  </i>
                                          Delete </button>
    ...
    <div class="w-full rounded-xl bg-white p-4 shadow-2xl shadow-black/40" v-if="form">
     <div class="flex items-center justify-between mb-4">
                        <h3 class="text-xl font-bold leading-none text-gray-900">Edit/Create Metags</h3>
                        <div class="flex items-center">
    ...
    <div class="mb-4 grid grid-cols-2 gap-4">
      <div class="flex flex-col">
        <label for="text" class="mb-2 font-semibold">Page</label>
   ...
   <div class="mb-4 grid grid-cols-2 gap-4">
    <div class="flex flex-col">
        ...
        <div class="flex flex-col">
        <label for="text" class="mb-2 font-semibold">Keywords</label>
        <textarea  v-model="keywords" type="text" id="text" class="w-full max-w-lg rounded-lg border border-slate-200 px-2 py-1 hover:border-blue-500 focus:outline-none focus:ring focus:ring-blue-500/40 active:ring active:ring-blue-500/40"></textarea>
        </div>
    </div>
